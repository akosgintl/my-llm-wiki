<!-- SOURCE OF TRUTH for this vault's agent instructions.
     CLAUDE.md imports this file with @AGENTS.md, so Claude Code and Codex share it.
     .cursor/rules/second-brain.mdc is a MIRROR of everything below and must be kept in sync
     by hand — Cursor loads .mdc files directly and a cross-file include is not reliable there. -->

# document-intelligence

> A knowledge base on document intelligence — OCR, layout analysis, document parsing, and information extraction that turns unstructured documents into structured data.
...
Every wiki page MUST include YAML frontmatter:
    ...
    ---
    aliases: ["Page Title In Title Case"]
    tags: [tag1, tag2]
...
**Because of this, every page MUST declare `aliases: ["Its Title Case Title"]` in its frontmatter.** Obsidian resolves `[[Some Page]]` against the filename or an alias, and **hyphens are not equivalent to spaces** — so without the alias, `[[Golden Set and Eval Harness]]` does not resolve to `golden-set-and-eval-harness.md` and every internal link renders as a dead link. The alias must match the page's `# H1` exactly, and it is the string every other page links by.
...
5. Every wiki page must have YAML frontmatter with aliases, tags, sources, created, and updated fields. `aliases` must contain the page's exact Title Case H1 — without it, wikilinks do not resolve in Obsidian.
